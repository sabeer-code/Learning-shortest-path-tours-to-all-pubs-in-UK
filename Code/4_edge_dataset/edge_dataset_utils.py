...
def edges2csv(graph_dir, edges_dir, metric='euclid'):
    ...
    for i in range(num_files):
        df_edges = pd.read_csv(edges_dir + 'Graph{}.edges'.format(i), delimiter=' ', skiprows=[0], names=['Node1_ID', 'Node2_ID', 'EUCLIDEAN_ROUNDED_DISTANCE'])
        df_graph = pd.read_csv(graph_dir + '/Graph{}.tsp'.format(i), delimiter=' ', skiprows=[j for j in range(6)], names=['NODE_ID', 'LONGITUDE', 'LATITUDE'])
        ...
        elif metric is 'geodesic':
            df_edges['GEODESIC_ROUNDED_DISTANCE_KM'] = pd.Series(list(zip(df_edges['NODE1_COORDS'], df_edges['NODE2_COORDS']))).apply(lambda x: np.round(geodesic(x[1], x[0]).kilometers)) # Geodesic rounded distance between Coordinates
